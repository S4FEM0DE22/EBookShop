...
function showOrder(id) {
  ...
  showDialog(`<div class="dialog-head"><h2>รายละเอียดคำสั่งซื้อ</h2><button type="button" data-close-dialog aria-label="ปิด">×</button></div><p class="order-id">${escapeHtml(order.id)} <button class="mini" id="copy-order-id" type="button">คัดลอกเลข</button></p><p>${escapeHtml(order.customerName)} · ${escapeHtml(order.email)}</p><p>สร้างเมื่อ ${formatDate(order.createdAt)} · สถานะ ${statusText(order.status)} · ${emailText(order.emailStatus)}</p><div class="dialog-items">${order.items.map(item => `<div><img src="${escapeHtml(item.cover)}" alt=""><span>${escapeHtml(item.title)}</span><strong>${item.price} บาท</strong></div>`).join('')}</div><p class="dialog-total">ยอดรวมจำลอง <strong>${order.price} บาท</strong></p><button class="mini" data-close-dialog type="button">ปิด</button>`);
...
function render() {
  ...
  app.innerHTML = `<div class="admin-shell"><header class="site-header"><a class="brand" href="/" aria-label="SAFE MODE SHOP หน้าร้าน"><img src="/assets/safe-mode-shop-white.png" alt="SAFE MODE SHOP"></a><nav class="nav-pills" aria-label="เมนูผู้ดูแล"><button data-view="overview" class="${view === 'overview' ? 'active' : ''}">ภาพรวม</button><button data-view="orders" class="${view === 'orders' ? 'active' : ''}">คำสั่งซื้อ</button><button data-view="books" class="${view === 'books' ? 'active' : ''}">หนังสือ</button><button data-view="customers" class="${view === 'customers' ? 'active' : ''}">ลูกค้า</button><a href="/">หน้าร้าน</a><button class="nav-exit" data-action="logout" aria-label="ออกจากระบบ" title="ออกจากระบบ"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M14 17l5-5-5-5"/><path d="M19 12H7"/><path d="M12 3H6a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h6"/></svg></button></nav></header><main><div class="page-heading"><span class="eyebrow">SAFE MODE SHOP / ADMIN</span><h1>${title}</h1></div><section class="admin-content"><div class="content-head"><div><span class="eyebrow">ADMIN WORKSPACE</span><h2>${view === 'overview' ? 'จัดการร้าน E-book' : view === 'orders' ? 'รายการการสั่งซื้อทั้งหมด' : view === 'books' ? 'หนังสือทั้งหมด' : 'บัญชีลูกค้า'}</h2></div><div class="content-actions"><span class="demo-badge">DEMO ONLY · ไม่มีการรับเงินจริง</span><button class="mini" data-action="refresh">รีเฟรชข้อมูล</button></div></div>${view === 'overview' ? `${metrics()}<section class="panel"><div class="panel-head"><div><h2>คำสั่งซื้อล่าสุด</h2><p>5 รายการล่าสุด</p></div><button class="mini" data-view="orders">ดูทั้งหมด →</button></div>${orderTable(5)}</section>` : view === 'orders' ? `<section class="panel"><div class="panel-head"><div><h2>ติดตามคำสั่งซื้อ</h2><p>แสดงสูงสุด 100 รายการล่าสุด</p></div><div class="order-filters"><select class="field" id="status-filter" aria-label="กรองสถานะ"><option value="ALL" ${statusFilter === 'ALL' ? 'selected' : ''}>ทุกสถานะ</option><option value="PENDING" ${statusFilter === 'PENDING' ? 'selected' : ''}>รอชำระ</option><option value="PAID" ${statusFilter === 'PAID' ? 'selected' : ''}>ชำระแล้ว</option><option value="CANCELLED" ${statusFilter === 'CANCELLED' ? 'selected' : ''}>ยกเลิกแล้ว</option></select><input class="field search" id="order-search" type="search" placeholder="ค้นหาเลขคำสั่งซื้อ ชื่อ หรืออีเมล" value="${escapeHtml(query)}" aria-label="ค้นหาคำสั่งซื้อ"></div></div><div id="order-results">${orderTable(100)}</div></section>` : view === 'books' ? booksPanel() : customersPanel()}</section></main><footer class="site-footer">SAFE MODE SHOP · ADMIN WORKSPACE</footer></div>`;
