...
function setView(html, active = '') {
  ...
  for (const link of navLinks) {
    if (link.dataset.nav === 'profile') link.hidden = !customerUser;
    if (link.dataset.nav === 'login' || link.dataset.nav === 'register') link.hidden = Boolean(customerUser);
  }
  navAuthAction.classList.toggle('signed-in', Boolean(customerUser));
  navAuthAction.setAttribute('aria-label', customerUser ? 'ออกจากระบบ' : 'เข้าสู่ระบบ');
  navAuthAction.title = customerUser ? 'ออกจากระบบ' : 'เข้าสู่ระบบ';
...
function authPage(mode = 'login', message = '') {
  ...
  const form = `<form id="auth-form">${formFields}<label for="auth-password">รหัสผ่าน</label><input id="auth-password" name="password" type="password" autocomplete="${register ? 'new-password' : 'current-password'}" required minlength="8" maxlength="128" placeholder="อย่างน้อย 8 ตัวอักษร"><div id="live-message" aria-live="polite"></div><button class="pill-button dark" type="submit">${register ? 'สร้างบัญชี' : 'เข้าสู่ระบบเพื่อซื้อ'}</button></form>`;
  const content = register
    ? `<div class="kicker">CREATE YOUR ACCOUNT</div><h1>สมัครสมาชิก</h1><p>สร้างบัญชีเพื่อสั่งซื้อ E-book และติดตามคำสั่งซื้อของคุณ</p>${form}<p class="auth-note">อาจต้องยืนยันอีเมลก่อนเข้าสู่ระบบ</p><a class="auth-switch" href="#login">มีบัญชีแล้ว? เข้าสู่ระบบ</a>`
    : `<div class="kicker">WELCOME BACK</div><h1>เข้าสู่ระบบ</h1><p>เข้าสู่ระบบเพื่อสั่งซื้อ E-book และดูคำสั่งซื้อของคุณ</p>${message ? notice(message, message.startsWith('ลิงก์ยืนยัน') ? 'error' : 'success') : ''}${form}<p class="auth-note"><a href="#forgot-password">ลืมรหัสผ่าน?</a></p><a class="auth-switch" href="#register">ยังไม่มีบัญชี? สมัครสมาชิก</a>`;
  setView(`<section class="profile-layout auth-layout"><div class="white-panel profile-card auth-card">${content}</div><div class="profile-visual"><div class="avatar-graphic" aria-hidden="true"><span></span></div><p>SAFE MODE SHOP</p></div></section>`, register ? 'register' : 'login');
...
function profile() {
  if (!customerUser) { location.hash = '#login'; return authPage(); }
...
navAuthAction.addEventListener('click', () => { if (customerUser) logoutCustomer(navAuthAction); else { location.hash = '#login'; authPage(); } });
