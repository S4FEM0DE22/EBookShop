...
export function makeDownloadToken(order, bookId = order.book_id) {
  const payload = Buffer.from(JSON.stringify({ id: order.id, bookId, exp: Math.floor(Date.now() / 1000) + 86400 })).toString('base64url');
...
export function downloadUrl(order, origin, bookId = order.book_id) {
  return `${origin}/api/download?token=${encodeURIComponent(makeDownloadToken(order, bookId))}`;
}

export async function deliverEmail(order, books, origin) {
  const items = Array.isArray(books) ? books : [books];
  const downloadUrls = Object.fromEntries(items.map(book => [book.id, downloadUrl(order, origin, book.id)]));
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from,
      to: [order.email],
      subject: `ลิงก์ดาวน์โหลด E-book คำสั่งซื้อ ${order.id} (Demo)`,
      text: `สวัสดี ${order.customer_name}\n\nคำสั่งซื้อ ${order.id} มีสถานะ PAID (การชำระเงินจำลอง)\n\n${items.map(book => `${book.title}: ${downloadUrls[book.id]}`).join('\n')}\n\nลิงก์ข้างต้นใช้ได้ 24 ชั่วโมง ลิงก์ไฟล์จริงมีอายุ 5 นาทีหลังเปิด\nDEMO ONLY - ไม่มีการรับเงินจริง`
    })
  });
  if (!response.ok) {
