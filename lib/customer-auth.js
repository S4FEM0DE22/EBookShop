...
export function sessionCookie(request, user) {
  const payload = Buffer.from(JSON.stringify({ id: user.id, email: cleanEmail(user.email), name: user.name || '', username: user.username || null, version: user.version, exp: Math.floor(Date.now() / 1000) + lifetime })).toString('base64url');
...
export async function customer(request) {
  ...
  try {
    ...
    return { id: data.id, email: cleanEmail(data.email), name: typeof data.name === 'string' ? data.name : '', username: profile.username || null, version: data.version };
  } catch { return null; }
}
...
async function localLogin(identifier, password) {
  ...
  return { id: user.id, username: user.username || null, name: user.name, email: user.email, version: user.session_version || 1 };
}
...
export async function register(username, email, password) {
  ...
  return { user: { id: user.id, email: cleanEmail(user.email), username: profile?.username || username, name: username, version: profile?.session_version || 1 }, confirmationRequired: !result.access_token };
}
export async function login(identifier, password) {
  ...
  return { id: result.user.id, email: cleanEmail(result.user.email), username: profile.username, name: result.user.user_metadata?.full_name || profile.username || '', version: profile.session_version };
}
