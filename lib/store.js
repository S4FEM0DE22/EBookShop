...
export async function listAllBooks() {
  ...
  try { details = JSON.parse(await readFile(localBookDetailsFile, 'utf8')); }
  catch (error) { if (error.code !== 'ENOENT') throw error; }
  return books.map(book => ({ ...book, ...details[book.id], active: visibility[book.id] !== false }));
}

export async function updateBookDetails(id, changes) {
  if (!isLocalDemo()) return (await supabase('books', 'PATCH', `?id=eq.${encodeURIComponent(id)}`, changes))[0] || null;
  const work = localBooksQueue.then(async () => {
    if (!books.some(book => book.id === id)) return null;
    let details = {};
    try { details = JSON.parse(await readFile(localBookDetailsFile, 'utf8')); }
    catch (error) { if (error.code !== 'ENOENT') throw error; }
    details[id] = { ...details[id], ...changes };
    await mkdir(path.dirname(localBookDetailsFile), { recursive: true });
    await writeFile(localBookDetailsFile, JSON.stringify(details, null, 2));
    return (await listAllBooks()).find(book => book.id === id);
  });
