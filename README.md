...
เว็บร้าน E-book สาธิตตามใบงาน Vibe Coding 2026 ปรับหน้าตาตามภาพต้นแบบ SAFE MODE SHOP: โทนขาวดำ, เมนูทรงแคปซูล, หน้าแรกแบบสไลด์, แค็ตตาล็อก, ตะกร้า, ติดตามคำสั่งซื้อ และโปรไฟล์ลูกค้า ลูกค้าต้องสมัครหรือเข้าสู่ระบบก่อน Checkout ระบบใช้ Supabase Auth ในเว็บจริงและบัญชีจำลองที่เก็บรหัสผ่านแบบ scrypt ในเครื่อง ประวัติคำสั่งซื้อผูกกับอีเมลบัญชีและดูได้หลังกลับมาเข้าสู่ระบบ แค็ตตาล็อกใช้ PDF ที่ผู้ใช้ให้มา 4 ไฟล์ เลือกซื้อพร้อมกันได้สูงสุด 4 เล่ม แล้ว Checkout เป็นคำสั่งซื้อ `PENDING` หน้าติดตามแสดงรายการเป็นแถวพร้อมปุ่มชำระหรือยกเลิก เปลี่ยนสถานะเป็น `PAID` หรือ `CANCELLED` และแสดงลิงก์ดาวน์โหลดของแต่ละเล่มหลัง PAID ไม่มีการรับเงินจริง
...
1. สร้าง Supabase project แล้วรัน [schema.sql](supabase/schema.sql) ใน SQL Editor จะได้ตาราง `books`, `orders` ที่เปิด RLS และ private Storage bucket ชื่อ `ebooks`
   เปิด Authentication > Providers > Email สำหรับบัญชีลูกค้า และตั้ง Authentication > URL Configuration > Site URL เป็น `https://safemode-shop.vercel.app/` เพื่อให้ลิงก์ยืนยันอีเมลกลับมาที่ร้าน ตามค่าเริ่มต้นของ Supabase ลูกค้าต้องยืนยันอีเมลก่อนเข้าสู่ระบบ การส่งเมลยืนยันนี้เป็นของ Supabase Auth แยกจาก Resend ที่ใช้ส่ง E-book
